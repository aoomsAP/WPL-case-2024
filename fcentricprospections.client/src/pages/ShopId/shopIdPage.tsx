...
import { Shop, IProspection } from '../../types'
        ...
        <section className={styles.prospectionSection}>
          ...
          <ul>
            {shopProspections.map(prospection => (<li className={styles.li} key={prospection.id}>  {/* Ensure each `li` has a unique `key` */}
              <Link className={styles.prospectionA} to={`/shop/${id}/prospection/${prospection.id}`}>Prospectie {prospection.date?.toString().slice(0, 10)}<FaAngleRight className={styles.icon} /> </Link></li>))}
          </ul>
          <button className={styles.button}>
